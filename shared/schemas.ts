...
export const MemberSchema = z.object({
  ...
  hiddenConnectionAccountIds: z.array(z.string()).optional(),
  showChildTodosInOwnView: z.boolean().optional(),
  ...
  taskTemplateOrder: z.array(IdSchema).optional(),
  familyThreadOrder: z.array(IdSchema).optional(),
  ...
  shoppingShowCompletedDefault: z.boolean().optional(),
...
export const MemberPatchSchema = MemberSchema.pick({
  ...
  darkMode: true,
  textSize: true,
  hiddenCrossAccountIds: true,
  hiddenConnectionAccountIds: true,
  ...
  taskTemplateOrder: true,
  familyThreadOrder: true,
  todoThreadRange: true,
  todoThreadGap: true,
  todoBubbleSize: true,
  shoppingShowCompletedDefault: true,
  spentStars: true
}).partial();
...
export const TodoSubtaskSchema = z.object({
  ...
  // Servern äger detta fält (se shared/types.ts) — accepteras här bara så
  // att den befintliga generiska PATCH-vägen (TodoPatchSchema.subtasks) inte
  // avvisar den array klienten redan har i sitt state, värdet skrivs alltid
  // över server-side (todosService.ts:s updateTodo).
